...
class WorkflowTemplate extends ContentEntityBase {
  ...
  public static function baseFieldDefinitions(EntityTypeInterface $entity_type) {
    ...
    // Reference to workflow_list config entities that make up this template.
    $fields['template_workflows'] = BaseFieldDefinition::create('entity_reference')
      ->setLabel(t('Template Workflows'))
      ->setDescription(t('The workflow lists included in this template.'))
      ->setRevisionable(TRUE)
      ->setSetting('target_type', 'workflow_list')
      ->setSetting('handler', 'default')
    ...
    $fields['assignments'] = BaseFieldDefinition::create('entity_reference')
      ->setLabel(t('Legacy Assignments'))
      ->setDescription(t('Legacy field - use template_workflows instead.'))
      ->setRevisionable(TRUE)
  ...
  /**
   * Gets the workflow list IDs in this template.
   *
   * @return array
   *   Array of workflow list IDs.
   */
  public function getTemplateWorkflows() {
    $workflows = [];
    foreach ($this->get('template_workflows') as $item) {
      if ($item->target_id) {
        $workflows[] = $item->target_id;
      }
    }
    return $workflows;
  }
  ...
  /**
   * Gets the count of workflows in this template.
   *
   * @return int
   *   The number of workflows.
   */
  public function getWorkflowCount() {
    return count($this->get('template_workflows'));
  }

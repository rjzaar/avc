...
class WorkflowAssignmentSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['notifications']['enable_notifications'] = [
      ...
      '#description' => $this->t('Send email notifications to assigned users/groups when workflows are assigned to content.'),
    ];
      ...
      if (in_array($type, $enabled_types)) {
        ...
        if (!$field) {
          ...
          $form_display->setComponent('field_workflow_list', [
            // Use options_select to avoid checkbox zero value issues
            'type' => 'options_select',
            'weight' => 100,
            'settings' => [],
          ])->save();
